...
export default function ImageField(props: ImageFieldProps) {
  ...
  const [uploadTmp] = useTempFileUploadMutation()
  const [removeTmp] = useTempFileRemoveMutation()
  ...
  async function sequentialUpload(images: ImageObject[]) {
    ...
    for (const image of images) {
      ...
      try {
        const payload = await uploadTmp(data).unwrap()
        const path = payload
  ...
  async function handleRemove(path: string) {
    try {
      const res = await removeTmp(path).unwrap()
